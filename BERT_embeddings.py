...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 11:44:14 2020

@author: shihao

This script uses a pretrained BERT model to generate word embeddings
"""
...
#%%
# load dataset
with open('dataset/merged_whole_sentences_no_stops.json', 'r') as json_file:
    X = json.load(json_file)
    
#%%
# down-sampling for experiments
down_ratio = 1
X[:] = X[::down_ratio]
#%%
# add marks and tokenize
X_tokenized = [[tokenizer.tokenize('[CLS] ' + text + ' [SEP]'), weight] for text, weight in X]
#%%
# convert tokens into indices
X_indexed = [[tokenizer.convert_tokens_to_ids(tokens), weights] for tokens, weights in X_tokenized]
#%%
# save BERTinized data
# with open('dataset/merged_bertized.json', 'w') as json_file:
#     json.dump(X_indexed, json_file)
...
merged[:] = merged[::down_ratio]
#%%
# select only captions
num_captions = 3150
X[:] = X[:num_captions]
...
indices, weight = X_indexed[rand_idx]
text_tokens = X_tokenized[rand_idx][0]
